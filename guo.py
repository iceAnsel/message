...
# Server酱配置
sckey = 'SCT276724TEvqg3ub3us4ThBPylxQ4ujkv'  # 替换为你的SCKEY

SERVER_CHAN_KEY = 'SCT276712TJnCoiKggijgh3U2bMqbz46Rd'
API_KEY = "11ecf4bbed19f6ddacdbaffd233e92a5"
CONSTELLATION_ID = 10  # 星座ID（水瓶座）

# SCT276724TEvqg3ub3us4ThBPylxQ4ujkv  郭颖
...
def format_markdown(weather_data, almanac_data, constellation_data):
    ...
    # 纪念日计算
    birthday = datetime.date(2005, 1, 15)
    next_birthday = datetime.date(today.year+1, 1, 15) if today.date() > birthday.replace(year=today.year) else birthday.replace(year=today.year)
    love_days = (today.date() - datetime.date(2025, 4, 7)).days
    ...
    - ⏳ 生日倒计 ➠ {(next_birthday - today.date()).days}天 | 距离♑摩羯月还有{( datetime.date(2025, 12, 22) -today.date() ).days}天  
    &ensp; &emsp; &nbsp;  
    🥛 晨间密语：今日幸运早餐组合 ➠ 豆浆+饭团（加肉松概率提升30%）  
    &ensp; &emsp; &nbsp;  
    📍 推荐店铺：广厦早餐铺（距您1.2km 🚴♀️8分钟可达）  
    """
...
def get_progress_bar(percentage, length=5):
    """生成可视化进度条"""
...
def weather_report(city):
    ...
    constellation_result = get_constellation("11ecf4bbed19f6ddacdbaffd233e92a5", 10)
    ...
    send_serverchan(f"🌤️💖 郭颖小老婆~ 的每日提醒 💌 ", markdown_content)
