...
def format_markdown(weather_data, almanac_data, constellation_data):
    ...
    # 纪念日计算
    birthday = datetime.date(2003, 1, 7)
    next_birthday = datetime.date(today.year+1, 1, 7) if today.date() > birthday.replace(year=today.year) else birthday.replace(year=today.year)
...
def weather_report(city):
    ...
    send_serverchan(f"🌤️💖 Good Morning Sir 🤖 ", markdown_content)
